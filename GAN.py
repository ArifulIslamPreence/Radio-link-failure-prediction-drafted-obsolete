...
dimension = int(len(features))
data_len = len(train.rlf.values)
batchSize = 100
df1 = df1.interpolate(method='linear', limit_direction='forward')
train.fillna(train.mean(), inplace=True)
test.fillna(test.mean(), inplace=True)
label = train.rlf.values
...
def training_gan(generator, discriminator, num_epochs, Batch_size, train_loader):
    for epoch in range(num_epochs):
        ...
        for n, data in enumerate(train_loader):
            ...
            real_samples = data  # The data size should be (100,35) as batch size. Its showing (33,35). why?
            real_samples_labels = label_tensor
            ...
            output_discriminator_generated = discriminator(
                generated_sample)  # D decision array for suucessful distinguishing
            # torch.reshape(real_samples_labels,())
